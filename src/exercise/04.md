...
* Functions in React are called "components" - they return something that is "renderable".
* Components start with a capitalized letter, if they return a custom made renderable component such as `<SpecificComponent>`.
* If a component starts with a lowercase letter, it has to return a builtin component such as `<div>`.
## Background
