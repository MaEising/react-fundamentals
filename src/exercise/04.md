...
- Functions in React are called "components" - they return something that is
  "renderable".
- Components start with a capitalized letter, if they return a custom made
  renderable component such as `<SpecificComponent>`.
- If a component starts with a lowercase letter, it has to return a builtin
  component such as `<div>`.
- Calling custom components from JSX works like this
  `ui = <lower_snake_case />`and Babel compiles it to
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=eising%40sipate.de).
